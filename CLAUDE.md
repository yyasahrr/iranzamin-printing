...
- **Theming:** dark/light via `data-theme` on `<html>` and CSS variables in `frontend/src/index.css`.
- **Auth state:** `frontend/src/contexts/AuthContext.tsx` (demo persists to localStorage; swap for Supabase/WooCommerce in production).
...
That target runs `npm run build` inside `frontend`, which executes TypeScript type-checking and the Vite production build. If `make check` fails, the repo is NOT consistent — fix it before doing anything else.
...
- **MUST NOT** edit `frontend/package.json` or `frontend/vite.config.ts` directly. Use the package installer tool for new dependencies.
...
- **MUST** keep styling framework-agnostic (plain CSS classes + CSS variables in `frontend/src/index.css`) so it can port to an Elementor/WordPress widget.
...
- **Layer 3 (system confirmation):** full build output (`frontend/dist/index.html`) is produced and serves.
...
- `frontend/src/App.tsx` — landing page (single component).
- `frontend/src/pages/` — routed pages (Login, Profile, PortfolioDetail).
- `frontend/src/contexts/AuthContext.tsx` — auth provider (single source of truth for the logged-in user).
- `frontend/src/data/portfolio.ts` — portfolio content (machine-readable source of truth for projects).
- `frontend/src/index.css` — all styles (CSS variables + component classes).
- See `frontend/src/ARCHITECTURE.md` for the module-level design.
