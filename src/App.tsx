...
function App() {
  ...
  const estimatedPrice = useMemo(() => {
    const finishFactor = details.finishes.reduce((sum, finishId) => {
      return sum + (finishes.find((item) => item.id === finishId)?.factor ?? 0);
    }, 0);
    const sideFactor = details.printSide === "دو رو رنگی" ? 1.16 : 1;
    const price = product.unitPrice * details.quantity * (1 + finishFactor) * sideFactor;
    return Math.round(price / 10000) * 10000;
  }, [details.finishes, details.printSide, details.quantity, product.unitPrice]);
  ...
  const submitOrder = () => {
    ...
    // This local fallback is replaced by the WooCommerce or Supabase adapter in production.
    try {
      const previousOrders = JSON.parse(localStorage.getItem("chap-roshan-orders") ?? "[]");
      localStorage.setItem("chap-roshan-orders", JSON.stringify([...previousOrders, order]));
    } catch {
      // The integration event still lets a host such as Elementor persist the order.
    }

    window.dispatchEvent(new CustomEvent("chap-roshan:order-created", { detail: order }));
    setTrackingCode(new Intl.NumberFormat("fa-IR", { useGrouping: false }).format(Number(orderId.slice(3))));
    setOrderSubmitted(true);
  };

  const submitOrgRequest = (event: React.FormEvent) => {
    ...
    if (!/^0\d{9,10}$/.test(orgForm.phone)) {
      setOrgError("شماره تماس معتبر (موبایل یا ثابت با کد شهر) وارد کنید.");
      return;
    }
    const request = { ...orgForm, createdAt: new Date().toISOString() };
    // Demo fallback — replaced by the WooCommerce/Supabase adapter in production.
    try {
      ...
      // The integration event still lets the host (Elementor/WordPress) persist the request.
    }
  ...
  return (
    <div>
      <div className="step-title">
        ...
        <p>این گزینه‌ها بعداً توسط کارشناس قابل اصلاح هستند.</p>
      </div>
  ...
  return (
    <div>
      <div className="step-title">
        ...
        <p>فایل را می‌توانید حالا یا پس از تماس کارشناس بفرستید.</p>
      </div>
      <label className="file-upload">
        ...
        <span>
          ...
          <small>{fileName ? "برای تغییر فایل دوباره کلیک کنید" : "PDF، AI، TIFF یا JPG تا ۵۰ مگابایت"}</small>
        </span>
      </label>
