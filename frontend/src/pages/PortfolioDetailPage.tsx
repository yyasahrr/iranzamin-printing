...
export default function PortfolioDetailPage() {
  ...
  const localWork = getPortfolioBySlug(slug ?? "");
  const [work, setWork] = useState<any>(localWork);
  ...
  // بارگذاری داینامیک جزئیات نمونه کار از بک‌اند جنگو
  useEffect(() => {
    let isMounted = true;
    async function loadProject() {
      try {
        const res = await fetch(`http://localhost:8000/api/orders/portfolio/${slug}/`);
        if (res.ok) {
          ...
          if (isMounted) {
            setWork({
              ...
              title: p.title,
              category: p.category,
              image: p.cover_image || localWork?.image || "/images/work-cafe.jpg",
              gallery: p.gallery_images.length > 0 
                ? p.gallery_images.map((g: any) => g.image) 
                : localWork?.gallery || [p.cover_image || "/images/work-cafe.jpg"],
              client: p.client,
              year: localWork?.year || "۱۴۰۴",
              description: p.description || localWork?.description || "",
              fullDescription: p.description || localWork?.fullDescription || "",
              specs: localWork?.specs || [
                { label: "مشتری", value: p.client },
                { label: "دسته‌بندی", value: p.category }
              ],
              services: localWork?.services || ["چاپ دقیق افست"],
              testimonial: p.testimonial ? {
                text: p.testimonial.text,
                author: p.testimonial.author,
                role: p.testimonial.role
              } : localWork?.testimonial
            });
          }
        }
      } catch (error) {
        console.warn("Could not fetch portfolio detail from Django. Using local fallback.");
      }
    }
    loadProject();
    return () => {
